...
class BadgeAwarder
{
    ...
    private function getEndingPeriods(string $today): array
    {
        ...
        $dayOfWeek = (int)gmdate('N', strtotime($today)); // 1=Mon, 7=Sun
        if ($dayOfWeek === 1) { // Monday = new week started, award last week
            $lastSunday  = gmdate('Y-m-d', strtotime($today . ' -1 day'));
            $lastMonday  = gmdate('Y-m-d', strtotime($today . ' -7 days'));
        ...
        if (gmdate('j', strtotime($today)) === '1') {
            $firstOfLastMonth = gmdate('Y-m-01', strtotime($today . ' -1 month'));
            $lastOfLastMonth  = gmdate('Y-m-t', strtotime($today . ' -1 month'));
            $periods[] = [Types::PERIOD_MONTHLY, $firstOfLastMonth, $lastOfLastMonth];
        }

        // Quarterly: Jan/Apr/Jul/Oct 1
        $month = (int)gmdate('n', strtotime($today));
        $day   = (int)gmdate('j', strtotime($today));
        if ($day === 1 && in_array($month, [1, 4, 7, 10], true)) {
            $qStart = gmdate('Y-m-d', strtotime($today . ' -3 months'));
            $qEnd   = $yesterday;
        ...
        // Annual: Jan 1
        if ($month === 1 && $day === 1) {
            $aStart = gmdate('Y-m-d', strtotime($today . ' -1 year'));
            $aEnd   = $yesterday;
