    ...
    private function escalateStep2(): void
    {
        ...
        foreach ($checkins as $checkin) {
            try {
                ...
                // Escalation messages mapped correctly to personality keys
                $msgs = [
                    Types::PERSONALITY_HYPE      => "Hey <@{$checkin['user_id']}>! We know you've got this — just let us know! 🔥",
                    Types::PERSONALITY_DRY       => "Check-in for <@{$checkin['user_id']}> not yet recorded. Still waiting.",
                    Types::PERSONALITY_SARCASTIC => "Hey <@{$checkin['user_id']}>, still waiting — did this happen or not?",
    ...
    private function processStep3Misses(): void
    {
        ...
        $checkins = Database::fetchAll(
            "SELECT c.*, g.personality, g.name AS goal_name, g.user_id, g.id AS goal_id,
                    g.guild_id, g.streak_count, g.cadence_type, gm.display_name, sc.accountability_channel_id
               FROM checkins c
               JOIN goals g ON g.id = c.goal_id
               JOIN guild_members gm ON gm.guild_id = g.guild_id AND gm.user_id = g.user_id
               JOIN server_config sc ON sc.guild_id = g.guild_id
        ...
        foreach ($checkins as $checkin) {
            try {
                if ($checkin['cadence_type'] !== Types::CADENCE_DAILY) {
                    // Weekly, monthly, and one-time goals: mark missed silently in DB
                    Database::execute(
